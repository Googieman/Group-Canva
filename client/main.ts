...
async function startEditor(rootElement: HTMLElement, localStorage: CanvasStorage, roomId: string | null, fileId: string | null, hostFileId: string | null): Promise<void> {
  ...
  if (!fileId || hostFileId) {
    connection = new Connection(state, sessionRoomId!, isHostSession ? 'Host' : `Guest ${Math.floor(Math.random() * 9000 + 1000)}`, {
      ...
      error(message) { ui.notify(message); }, connectionError(message) { enabled = false; discardLocal(); ui.setConnectionError?.(message); render(); }, ping(label) { ui.setPing(label); }, hostSave(watermark) { if (!isHostSession) ui.setSaveLabel(watermark ? 'Saved by host' : 'Waiting for host save'); }, roomStatus(value) { enabled = value.status === 'active' && (!localWriter || !localWriter.readOnly) && (!isHostSession || !hostRestorePending); if (value.status === 'paused' && isHostSession) void saveNow(); if (value.status === 'ended') { ui.setHost(false); ui.setReadOnly(true); } if (value.status !== 'active') discardLocal(); if (board) board.setEnabled(enabled); if (value.status === 'paused') ui.setConnection('disconnected', value.message ?? 'Waiting for the host to reconnect…'); else if (value.status === 'ended') ui.setConnection('disconnected', value.message ?? 'Session ended.'); else if (value.status === 'active') { ui.setConnection('connected'); render(); } },

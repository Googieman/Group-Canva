...
import { applyDocumentCommand, createHistory, documentToLegacyStrokes, objectBounds, type CanvasObject, type DocumentCommand, type Transaction } from '../shared/document';
...
import { canvasToPng } from './export';
...
async function startEditor(rootElement: HTMLElement, localStorage: CanvasStorage, roomId: string | null, fileId: string | null, hostFileId: string | null): Promise<void> {
  ...
  if (fileId || hostFileId) {
    ...
    state.hydrate({ epoch: `local-${localFile.id}`, revision: localFile.revision, roomId: localFile.id, selfId, strokes: documentToLegacyStrokes(localFile.document), redoIds: [], users: [{ id: selfId, name: 'You', color: '#5446d4' }], document: localFile.document });
    void hydrateAssets(localFile.assets);
    enabled = !isHostSession && !localWriter.readOnly; users = [{ id: selfId, name: 'You', color: '#5446d4' }]; if (localWriter.readOnly) ui.setReadOnly(true);
  }
  if (!fileId || hostFileId) {
    connection = new Connection(state, sessionRoomId!, isHostSession ? 'Host' : `Guest ${Math.floor(Math.random() * 9000 + 1000)}`, {
      ...
      error(message) { ui.notify(message); }, ping(label) { ui.setPing(label); }, hostSave(watermark) { if (!isHostSession) ui.setSaveLabel(watermark ? 'Saved by host' : 'Waiting for host save'); }, roomStatus(value) { enabled = value.status === 'active' && (!localWriter || !localWriter.readOnly); if (value.status === 'paused' && isHostSession) void saveNow(); if (value.status === 'ended') { ui.setHost(false); ui.setReadOnly(true); } if (value.status !== 'active') discardLocal(); if (board) board.setEnabled(enabled); if (value.status === 'paused') ui.setConnection('disconnected', value.message ?? 'Waiting for the host to reconnect…'); else if (value.status === 'ended') ui.setConnection('disconnected', value.message ?? 'Session ended.'); else if (value.status === 'active') { ui.setConnection('connected'); render(); } },
  ...
  function render(): void {
    ...
    const objects: CanvasObject[] = [...(state.document?.objects.filter(object => object.type !== 'ink') ?? [])]; if (draftObject) objects.push(draftObject); if (moveDelta.x || moveDelta.y) for (const index in objects) { const object = objects[index as unknown as number]; if (object && selectedIds.has(object.id)) objects[index as unknown as number] = { ...object, translation: { x: object.translation.x + moveDelta.x, y: object.translation.y + moveDelta.y } }; }
  ...
  function flush(): void { while (activeId && pending.length && enabled) { const points = pending.splice(0, BATCH_SIZE); if (!send({ type: 'stroke:points', id: activeId, offset, points })) break; offset += points.length; } }
  ...
  function ensureLocalDocumentMode(): void {
    if (localDocumentMode) return;
    localDocumentMode = true;
    const transactions = new Map<string, Transaction>();
    for (const object of state.document?.objects ?? []) {
      if (object.type !== 'ink' || !sessionInkIds.has(object.id) || !object.completed) continue;
      const after = structuredClone(object);
      after.active = true;
      transactions.set(object.id, { id: randomId('transaction'), authorId: selfId, kind: 'create', patches: [{ id: object.id, before: null, after }], timestamp: Date.now() });
    }
    const redo = state.redoIds.map(id => transactions.get(id)).filter((transaction): transaction is Transaction => !!transaction);
    const redoIds = new Set(redo.map(transaction => transaction.patches[0]?.id));
    state.documentHistory = {
      undo: [...transactions.values()].filter(transaction => !redoIds.has(transaction.patches[0]?.id) && transaction.patches[0]?.after !== null),
      redo,
    };
  }
  function recordLocalStrokeTransaction(id: string): void {
    const object = state.document?.objects.find(candidate => candidate.id === id);
    if (!object || object.type !== 'ink' || !object.completed || !state.documentHistory) return;
    const transaction: Transaction = { id: randomId('transaction'), authorId: selfId, kind: 'create', patches: [{ id, before: null, after: structuredClone(object) }], timestamp: Date.now() };
    state.documentHistory = { undo: [...state.documentHistory.undo, transaction], redo: [] };
  }
  function localStrokeCommand(command: Extract<Command, { type: `stroke:${string}` }> | { type: 'history:undo' } | { type: 'history:redo' }): boolean {
    ...
    else if (command.type === 'stroke:end') { const stroke = state.strokes.find(item => item.id === command.id); if (!stroke || stroke.completed) return false; change = { type: 'stroke:end', id: command.id, completionOrder: Math.max(0, ...state.strokes.map(item => item.completionOrder ?? 0)) + 1 }; }
    ...
    state.receive({ epoch: state.epoch, revision: changeRevision, change }); if (command.type === 'stroke:end' && localDocumentMode) recordLocalStrokeTransaction(command.id); render(); scheduleSave(); return true;
  }
  function localDocumentCommand(command: DocumentCommand): boolean {
    ...
    try {
      const before = command.type === 'history:undo' ? state.documentHistory.undo.at(-1) : command.type === 'history:redo' ? state.documentHistory.redo.at(-1) : undefined;
      ...
      const transaction = result.transaction ?? before; if (!transaction) return true;
  ...
  function openTextEditor(point: Point, value = ''): void { closeTextEditor(); textEditor = document.createElement('textarea'); textEditor.className = 'canvas-text-editor'; textEditor.value = value; textEditor.placeholder = 'Type here…'; const screen = worldToScreen(point, { width: BOARD_WIDTH, height: BOARD_HEIGHT }, board.getCamera()); textEditor.style.left = `${screen.x / BOARD_WIDTH * 100}%`; textEditor.style.top = `${screen.y / BOARD_HEIGHT * 100}%`; textEditor.style.fontSize = `${settings.fontSize * board.getCamera().zoom}px`; ui.canvas.parentElement?.append(textEditor); textEditor.focus(); }
  ...
  function updateDraft(point: Point, shift = false): void { if (!draftStart || !draftObject) return; if (draftObject.type === 'shape') { const geometry = constrainDrag(draftStart, point, draftObject.shape, shift); const x = Math.min(geometry.x, geometry.x + geometry.width), y = Math.min(geometry.y, geometry.y + geometry.height); draftObject = { ...draftObject, translation: { x, y }, width: Math.max(1, Math.abs(geometry.width)), height: Math.max(1, Math.abs(geometry.height)) }; } render(); }
  function commitDraft(): void { if (!draftObject) return; const object = draftObject; draftObject = null; draftStart = null; if (object.type === 'text') { const text = textEditor?.value ?? ''; closeTextEditor(); if (!text.trim()) { editingTextId = null; releaseSelectionLease(); render(); return; } if (editingTextId) { send({ type: 'object:text', id: editingTextId, text, expectedVersion: object.version, ...(leaseId ? { leaseId } : {}) }); releaseSelectionLease(); editingTextId = null; } else send({ type: 'object:create', object: { ...object, text } }); } else send({ type: 'object:create', object }); render(); }
  ...
  if (localFile) { ui.setFile(localFile.title, 'saved'); ui.setUsers(users, selfId); ui.setConnection('connected', 'Saved on this device'); board.setCamera(localFile.camera); } else ui.setUsers([], ''); board.setEnabled(enabled); render();

import './style.css';
import { BATCH_INTERVAL, BATCH_SIZE, BOARD_HEIGHT, BOARD_WIDTH, type AssetMeta, type Point, type Stroke, type User, type Command } from '../shared/protocol';
import { applyDocumentCommand, createHistory, documentToLegacyStrokes, legacyStrokesToDocument, nextObjectOrder, objectBounds, type CanvasObject, type DocumentCommand } from '../shared/document';
...
import { hitTestObject, constrainDrag, shapesHitByEraser } from './objects';
...
async function startEditor(rootElement: HTMLElement, localStorage: CanvasStorage, roomId: string | null, fileId: string | null, hostFileId: string | null): Promise<void> {
  ...
  const queuedAssetLoads: Array<() => void> = [];
  let activeAssetLoads = 0;
  const pumpAssetLoads = (): void => {
    while (activeAssetLoads < 3 && queuedAssetLoads.length) {
      const start = queuedAssetLoads.shift()!;
      activeAssetLoads++;
      start();
    }
  };
  const queueAssetLoad = (task: () => Promise<void>): Promise<void> => new Promise(resolve => {
    queuedAssetLoads.push(() => { void task().finally(() => { activeAssetLoads--; pumpAssetLoads(); resolve(); }); });
    pumpAssetLoads();
  });
  ...
  function loadRemoteAsset(meta: AssetMeta): Promise<void> {
    ...
    const existing = assetLoads.get(meta.id); if (existing) return existing;
    const load = queueAssetLoad(async () => {
      let bytes: ArrayBuffer | undefined;
      for (let attempt = 0; attempt < 3 && !bytes; attempt++) bytes = await connection?.downloadAsset(meta.id);
      if (!bytes) { ui.notify(`Image ${meta.id} is unavailable. Retry by refreshing the canvas.`); return; }
      rememberHostAsset(meta, bytes);
      try { loadedAssets.set(meta.id, await decodeImageAsset({ mimeType: meta.mimeType, bytes })); }
      catch { ui.notify(`Image ${meta.id} could not be decoded. Retry by refreshing the canvas.`); }
      if (board) render();
    }).finally(() => assetLoads.delete(meta.id));
    assetLoads.set(meta.id, load); return load;
  }
  function rememberHostAsset(meta: AssetMeta, bytes: ArrayBuffer): void {
    if (!isHostSession || !localFile || localWriter?.readOnly || localFile.assets.some(asset => asset.id === meta.id)) return;
    localFile = { ...localFile, assets: [...localFile.assets, { id: meta.id, mimeType: meta.mimeType, width: meta.width, height: meta.height, bytes }] };
    markDirty();
  }
  ...
  function localStrokeCommand(command: Extract<Command, { type: `stroke:${string}` }> | { type: 'history:undo' } | { type: 'history:redo' }): boolean {
    ...
    if (command.type === 'stroke:begin') { if (state.strokes.some(stroke => stroke.userId === selfId && !stroke.completed)) return false; const strokeOrder = Math.max(0, ...state.strokes.map(item => item.order)) + 1; const order = state.document ? Math.max(nextObjectOrder(state.document), strokeOrder) : strokeOrder; const stroke: Stroke = { id: command.id, userId: selfId, tool: command.tool, color: command.color, width: command.width, points: [command.point], order, completed: false, completionOrder: null, active: true }; change = { type: 'stroke:begin', stroke }; }
  ...
  function send(command: Command | { type: 'object:move'; ids: string[]; expectedVersions: Record<string, number>; operationId: string; leaseId?: string }): boolean {
    if (!enabled) return false;
    const normalizedCommand: Command = command.type === 'object:move' && !('delta' in command) ? { ...command, delta: moveDelta } : command;
    if (normalizedCommand.type === 'object:lease') return isSharedSession ? connection?.send(normalizedCommand) ?? false : true;
    if (isSharedSession) return connection?.send(normalizedCommand) ?? false;
    const historyCommand = normalizedCommand.type === 'history:undo' || normalizedCommand.type === 'history:redo';
    if (historyCommand && localDocumentMode) return localDocumentCommand(normalizedCommand as DocumentCommand);
    if (normalizedCommand.type.startsWith('stroke:') || historyCommand) return localStrokeCommand(normalizedCommand as Extract<Command, { type: `stroke:${string}` }> | { type: 'history:undo' } | { type: 'history:redo' });
    return localDocumentCommand(normalizedCommand as DocumentCommand);
  }
  ...
  saveCoordinator = createSaveCoordinator({
    ...
    write: async snapshot => {
      ui.setFile(snapshot.title, 'saving');
      try { await localStorage.saveFile(snapshot); }
      catch (error) { ui.setFile(snapshot.title, 'error', error instanceof Error ? error.message : 'Unable to save'); throw error; }
    },
    onSaved: snapshot => {
  ...
  function startDraft(point: Point): void { draftStart = point; if (settings.tool === 'text') { const existing = [...(state.document?.objects ?? [])].reverse().find(object => object.type === 'text' && hitTestObject(point, object)); if (existing?.type === 'text') { editingTextId = existing.id; selectedIds = new Set([existing.id]); acquireSelectionLease(); draftObject = existing; openTextEditor(existing.translation, existing.text); } else { editingTextId = null; openTextEditor(point); draftObject = { id: randomId('text'), type: 'text', order: nextObjectOrder(state.document!), version: 1, translation: point, text: '', width: 320, fontSize: settings.fontSize, color: settings.color, lineHeight: 1.25 }; } } else if (settings.tool === 'rectangle' || settings.tool === 'ellipse' || settings.tool === 'line' || settings.tool === 'arrow') { draftObject = { id: randomId('shape'), type: 'shape', order: nextObjectOrder(state.document!), version: 1, translation: point, shape: settings.tool, width: 1, height: 1, strokeColor: settings.color, strokeWidth: settings.width, fill: null }; } }
